...
- Name: public-endpoints.spec.ts >> Suite de Endpoints Públicos >> Ejecutar y validar todos los servicios abiertos de forma secuencial
...
```ts
  ...
  5  | 
  6  |     test('Ejecutar y validar todos los servicios abiertos de forma secuencial', async () => {
  7  |         
...
> 10 |             expect(loginRes.run.stats.assertions.failed).toBe(0);
  ...
  31 |         });
  32 | 
  33 |         test('Validar fallo de autenticación con credenciales incorrectas', async () => {
